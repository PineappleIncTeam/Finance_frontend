			...
			<div className={style.categories_name}>
				{isActive === "income" && !percentChoice && checkSum.length > 0 && (
					<div className={style.label_element}>
						<div className={style.category_color} style={{ backgroundColor: colorsStorage[1] }}></div>
						<div className={style.category_name}>
							Общий доход <span className={style.sum}>{numberFormatRub.format(checkSumTotal)} </span>
						</div>
					</div>
				)}
				{isActive === "income" &&
				...
				{isActive === "costs" && !percentChoice && checkSumOutTotal && (
					<div className={style.label_element}>
						<div className={style.category_color} style={{ backgroundColor: colorsStorage[1] }}></div>
						<div className={style.category_name}>
							Общий расход <span className={style.sum}>{numberFormatRub.format(totalCosts)} </span>
						</div>
					</div>
				)}
				{isActive === "costs" &&
						...
						return (
							item > 0 && (
								<div className={style.label_element} key={index}>
									...
									<div className={style.category_name}>
										{checkNameOut[index] || checkNameMoneyBox[index - (checkSumOut.length - checkNameMoneyBox.length)]}{" "}
										<span className={style.sum}>{!percentChoice ? numberFormatRub.format(item) : item + " %"} </span>
									</div>
								</div>
							)
...
export default ChartHistograms;
