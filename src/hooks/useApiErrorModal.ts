...
export function useApiErrorModal() {
	...
	const showError = useCallback((errorType: TApiErrorType, customMessage?: string) => {
		const errorConfig = {
			...
			cors: {
				title: "Запрос на перекрестный источник заблокирован",
				message:
					customMessage || "Браузер заблокировал запрос из-за ограничений безопасности. Пожалуйста, повторите позже.",
			},
			timeout: {
				title: "Тайм-аут запроса",
				message:
					customMessage ||
					"Выполнение запроса заняло слишком много времени. Проверьте подключение и повторите попытку.",
			},
		};
