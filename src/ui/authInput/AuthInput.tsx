...
const validateEmail = (value: string | boolean) => {
	...
	const invalidStartChars = /^[\s.-]/;
	if (invalidStartChars.test(stringValue)) {
	  return "Неверный формат ввода почты";
	}
	return true;
  };
...
const AuthInput = ({ label, type, placeholder, autoComplete, subtitle, error, ...props }: IAuthInput) => {
  ...
  return (
    <div className={styles.inputWrap}>
      ...
      <div className={cn(styles.inputWrap__element, { [styles.inputWrap__element_error]: fieldState.error })}>
        <input
          {...field}
